...
if [ "$SKIP_RUST" = false ]; then
  ...
  WASM_BUILD_TOOLCHAIN=nightly cargo test -p modnet-runtime -p modnet-node -p pallet-template --locked --release --features try-runtime -- --nocapture || log_error "Tests failed"
