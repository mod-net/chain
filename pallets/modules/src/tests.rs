...
fn module_data_length_enforced() {
    new_test_ext().execute_with(|| {
        ...
        let res: Result<BoundedVec<u8, <Test as crate::Config>::MaxStorageReferenceLength>, _> = BoundedVec::try_from(too_long);
        assert!(res.is_err());
...
fn register_module_emits_event_and_updates_storage() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), name.clone(), data_ref.clone(), url_ref.clone(), None));
        ...
        match &ev.event {
            RuntimeEvent::ModuleRegistry(Event::ModuleRegistered { who: e_who, id, name: e_name, data: e_data, url: e_url, collateral, take }) => {
                assert_eq!(*e_who, who);
                assert_eq!(*id, 0u64);
                assert_eq!(&e_name[..], &name[..]);
                assert_eq!(e_data.as_ref().map(|v| v.to_vec()), data_ref.as_ref().map(|v| v.to_vec()));
                assert_eq!(e_url.as_ref().map(|v| v.to_vec()), url_ref.as_ref().map(|v| v.to_vec()));
        ...
        assert_eq!(m.url.as_ref().map(|v| v.to_vec()), url_ref.as_ref().map(|v| v.to_vec()));
...
fn register_module_respects_take_and_max_take() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), name.clone(), data_ref.clone(), url_ref.clone(), Some(ok_take)));
        ...
        assert_noop!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(2), name2, sr(b"x"), url(b"u2"), Some(too_high)), Error::<Test>::MaxTakeExceeded);
    });
}
...
fn register_module_name_validation() {
    new_test_ext().execute_with(|| {
        let who: u64 = 1;
        // empty
        assert_noop!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), bv(b""), sr(b"x"), url(b"u"), None), Error::<Test>::NameEmpty);
        // leading space
        assert_noop!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), bv(b" foo"), sr(b"x"), url(b"u"), None), Error::<Test>::NameWhitespace);
        // trailing space
        assert_noop!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), bv(b"foo "), sr(b"x"), url(b"u"), None), Error::<Test>::NameWhitespace);
        // too long
        ...
        assert_noop!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), name, sr(b"x"), url(b"u"), None), Error::<Test>::NameNotUTF8);
        // name taken
        assert_ok!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), bv(b"dup"), sr(b"x"), url(b"u"), None));
        assert_noop!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(2), bv(b"dup"), sr(b"y"), url(b"u2"), None), Error::<Test>::NameTaken);
    });
}
...
fn update_module_works_and_checks_ownership_and_take() {
    new_test_ext().execute_with(|| {
        let who: u64 = 1;
        assert_ok!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), bv(b"a"), sr(b"x"), url(b"u"), None));
        // update name only
        assert_ok!(ModuleRegistry::<Test>::update_module(RuntimeOrigin::signed(who), 0, Some(bv(b"b")), None, None, None));
        let m = crate::Modules::<Test>::get(0).unwrap();
        assert_eq!(&m.name[..], b"b");
        // update url only
        assert_ok!(ModuleRegistry::<Test>::update_module(RuntimeOrigin::signed(who), 0, None, None, url(b"new-url"), None));
        let m = crate::Modules::<Test>::get(0).unwrap();
        ...
        assert_ok!(ModuleRegistry::<Test>::update_module(RuntimeOrigin::signed(who), 0, None, sr(b"y"), None, Some(new_take)));
        let m = crate::Modules::<Test>::get(0).unwrap();
        ...
        assert_noop!(ModuleRegistry::<Test>::update_module(RuntimeOrigin::signed(who), 0, None, None, None, Some(too)), Error::<Test>::MaxTakeExceeded);
        // ownership
        assert_noop!(ModuleRegistry::<Test>::update_module(RuntimeOrigin::signed(2), 0, Some(bv(b"c")), None, None, None), Error::<Test>::ModuleOwnership);
        // not found
        assert_noop!(ModuleRegistry::<Test>::update_module(RuntimeOrigin::signed(who), 999, None, None, None, None), Error::<Test>::ModuleNotFound);
    });
}
...
fn max_modules_reached() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(1), bv(b"a"), sr(b"x"), url(b"u1"), None));
        assert_ok!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(2), bv(b"b"), sr(b"y"), url(b"u2"), None));
        // Next is 2, saturating_add(1) == 3 which equals MaxModules -> error
        assert_noop!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(3), bv(b"c"), sr(b"z"), url(b"u3"), None), Error::<Test>::MaxModulesReached);
    });
}
...
fn module_url_length_enforced() {
    new_test_ext().execute_with(|| {
        ...
        let res: Result<BoundedVec<u8, <Test as crate::Config>::MaxURLLength>, _> = BoundedVec::try_from(too_long);
        assert!(res.is_err());

        // at bound is OK and registration succeeds
        let at_bound = vec![b'b'; max];
        let url_ok = url(&at_bound);
        assert_ok!(ModuleRegistry::<Test>::register_module(RuntimeOrigin::signed(who), name, data_ref, url_ok, None));
    });
}